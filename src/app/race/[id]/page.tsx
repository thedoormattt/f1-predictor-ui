...
export default async function RacePage({
  ...
  const [result, scores, predictions, players] = await Promise.all([
    getResult(raceId).catch(() => null),
    getRaceScores(raceId).catch(() => []),
    raceStarted
      ? getRacePredictions(raceId).catch(() => [])
      : Promise.resolve([]),
    fetch(
      `${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8000"}/players`,
    )
      .then((r) => r.json())
      .catch(() => []),
  ]);
  ...
  const playerMap = Object.fromEntries(
    players.map((p: any) => [p.id, p.username]),
  );
    ...
    <div className="space-y-8">
      ...
      {result ? (
        <div className="animate-fade-up card p-5 space-y-3">
          <h2 className="font-display font-bold text-xl uppercase tracking-wide text-f1red">
            Result
          </h2>
      ...
      {sortedScores.map((s, i) => {
        const pred = predictions.find(
          (p: Prediction) => p.player_id === s.player_id,
        );
        return (
          <ScoreRow
            key={s.player_id}
            ...
            prediction={pred}
            position={i + 1}
            playerName={playerMap[s.player_id] ?? s.player_id}
          />
        );
      })}
      ...
      {!raceStarted && (
        <div className="card p-5 text-center">
          <p className="font-mono text-f1muted text-sm">
            Predictions hidden until race starts
          </p>
        </div>
      )}
    </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-3">
        <div
          className={clsx("pos-badge", `pos-${position <= 3 ? position : "n"}`)}
        >
          {position}
        </div>
      ...
      <div className="grid grid-cols-4 gap-1 sm:grid-cols-6">
        {[
          ...
          >
            ...
            {pred && (
              <p className="driver-chip mx-auto mt-0.5 inline-block">{pred}</p>
            )}
            <p
              className={clsx(
                "font-display font-bold text-sm mt-0.5",
                pts > 0 ? "text-f1red" : "text-f1muted",
              )}
            >
              {pts > 0 ? `+${pts}` : "—"}
            </p>
