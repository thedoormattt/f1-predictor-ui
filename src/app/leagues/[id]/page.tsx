...
export default function LeaguePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    if (!user) return
    Promise.all([
      fetch(`${API}/leagues`, { headers: { 'X-Player-Id': user.id } })
        .then(r => r.json())
        .then((leagues: League[]) => leagues.find(l => l.id === leagueId) ?? null),
  ...
  if (!league) return (
    <div className="text-center py-16">
      <p className="font-display font-bold text-2xl uppercase text-f1muted">League not found</p>
    </div>
  )
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="animate-fade-up">
        ...
        <h1 className="font-display font-black text-5xl uppercase tracking-tight">{league.name}</h1>
        ...
        >
          <span className="uppercase tracking-widest">{league.invite_code}</span>
      ...
      <div className="animate-fade-up" style={{ animationDelay: '0.1s' }}>
        <h2 className="font-display font-bold text-2xl uppercase tracking-wide mb-4">Standings</h2>
      ...
      {cumulative.length > 0 && (
        <div className="animate-fade-up" style={{ animationDelay: '0.2s' }}>
          <h2 className="font-display font-bold text-2xl uppercase tracking-wide mb-4">Points Over Time</h2>
