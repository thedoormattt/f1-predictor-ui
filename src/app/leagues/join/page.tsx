...
export default function JoinLeaguePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API}/leagues/join?invite_code=${code.trim().toUpperCase()}`, {
        method: 'POST',
  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center">
      <div className="w-full max-w-sm animate-fade-up">
        <p className="font-mono text-f1red text-xs tracking-widest uppercase mb-1">Enter the pits</p>
        <h1 className="font-display font-black text-4xl uppercase tracking-tight mb-2">Join League</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1">
            <label className="font-mono text-xs text-f1muted uppercase tracking-wide">Invite Code</label>
            <input
              type="text"
              value={code}
