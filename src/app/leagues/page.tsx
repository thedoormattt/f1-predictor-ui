...
export default function LeaguesPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/leagues`, {
      ...
      .then(r => r.json())
      .then(data => { setLeagues(data); setFetching(false) })
  ...
  return (
    <div className="space-y-8">
      <div className="animate-fade-up">
        <p className="font-mono text-f1red text-xs tracking-widest uppercase mb-1">Your leagues</p>
        <h1 className="font-display font-black text-5xl uppercase tracking-tight">Leagues</h1>
      </div>
      ...
      {fetching ? (
        ...
        <div className="card p-8 text-center space-y-2">
          <p className="font-display font-bold text-xl uppercase text-f1muted">No leagues yet</p>
          <p className="font-mono text-f1muted text-sm">Create one or join with an invite code</p>
        </div>
      ) : (
          ...
          {leagues.map((league, i) => (
            ...
            >
              <div className="flex-1">
                <p className="font-display font-bold text-xl uppercase tracking-wide">{league.name}</p>
                <p className="font-mono text-f1muted text-xs mt-0.5">
                  Code: <span className="text-f1white tracking-widest">{league.invite_code}</span>
                </p>
              </div>
