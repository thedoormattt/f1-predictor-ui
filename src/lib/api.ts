import type {
  Race, Result, Prediction, Score,
  LeaderboardEntry, CumulativeEntry, Driver, Team
} from '@/types'
...
export const getRaceScores = (raceId: number) => get<Score[]>(`/results/${raceId}/scores`)

export const getRacePredictions   = (raceId: number)   => get<Prediction[]>(`/predictions/race/${raceId}`)
export const getPlayerPredictions = (playerId: string) => get<Prediction[]>(`/predictions/player/${playerId}`)
...
export const getCumulative  = () => get<CumulativeEntry[]>('/leaderboard/cumulative')
