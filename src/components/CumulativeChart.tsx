...
export default function CumulativeChart({ data }: { data: CumulativeEntry[] }) {
  ...
  const allRaces = [...new Set(data.map((d) => d.race_key))];
  const players = [...new Set(data.map((d) => d.player_name))];

  // Only show races where at least one player has a non-zero race score
  const races = allRaces.filter((race) =>
    data.some((d) => d.race_key === race && d.race_score > 0),
  );
  ...
  const rows: ChartRow[] = races.map((race) => {
    ...
    players.forEach((player) => {
      const entry = data.find(
        (d) => d.race_key === race && d.player_name === player,
      );
  ...
  return (
    <ResponsiveContainer width="100%" height={320}>
      <LineChart data={rows} margin={{ top: 8, right: 16, bottom: 8, left: 0 }}>
        <XAxis
          dataKey="race"
          tickFormatter={(key) => key.replace("_", " ")}
